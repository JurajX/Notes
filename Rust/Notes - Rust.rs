...
let vr: &[i32] = &v[1..];           // creates a slice starting at 1-st (0-indexed) element
...
struct Rectangle {                  // struct definition
    member: i64,                    // member field and its type
    number: u64,                    // member field and its type
}

impl Rectangle {                    // implementation block for associated functions; multiple impl blocks are allowed
    fn area(&self) -> u32 {         // member function
...
let st1 = Rectangle {                // instantiation of a struct
        member: 1,                  // assigning to the member
        number: 1,                  // assigning to the number
};
st1.number;                         // accessing the member

let st2 = Rectangle {                // another instance of struct
    member: 10,                     // assigning to the member
    ..st1                           // move the remaining fields from st1; st1 no longer usable
};

let sq = Rectangle::square(3);      // using the 'constructor'

#[derive(Debug)]                    // kinda decorator; helps with printing the struct with println!()
struct Another {/*...*/}            // just cefinition
